...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <script type="text/javascript">
  	...
  	$(function(){
		//分页开始
  		...
  		kkpager.init({
  			...
  			hrefFormer : '${pageContext.request.contextPath}/city!listData',//链接前部
  			hrefLatter : '.action',		 //链接尾部
  			getLink : function(n){
  				return this.hrefFormer + this.hrefLatter + "?pno="+n+"&cityCode=${cityCode}&cityName=${cityName}"; //+"&groupId="+groupId;
  			},
  			lang : {
  				prePageText : '上一页',
  				nextPageText : '下一页',
	...
	//按条件查询
  	function query(){
  		$("#totalForm").attr("action","${pageContext.request.contextPath}/city!listData.action");
	  	$("#totalForm").submit();
  	}
  	...
  	function addDataPage(){  		
  		$("#totalForm").attr("action","${pageContext.request.contextPath}/city!addDataPage.action");
	  	$("#totalForm").submit();
  	}
	...
	//删除	
  	function deleteData(){  		
		...
		if(checkboxs.is(":checked")){
			if(confirm("确定要删除所选中项吗?")){
				$("#totalForm").attr("action","${pageContext.request.contextPath}/city!deleteData.action");
				$("#totalForm").submit();
			}
		}else{
			alert("请选择删除项！");
  	...
  	function updateDataPage(){
  		...
  		if (ret == 0) {
			...
			$("#totalForm").attr("action","${pageContext.request.contextPath}/city!updateDataPage.action");
			$("#totalForm").submit();	
  		}
  	}
  	...
  	function changeStatus(status){
  		...
  		if(checkboxs.is(":checked")){
	  		...
	  		$("#totalForm").attr("action","${pageContext.request.contextPath}/city!updateDataBatch.action");
	  		$("#totalForm").submit();
  		}else{
  			alert("请选择修改项！");
  		}
		...
		<form action="" method="post" id="totalForm" >
			...
			<div class="content-box">
			    <div class="content-box-header">
			    	<span class="now_location">当前位置:</span>[城市表]
			        <div class="clear"></div>
			    </div>
			    <div class=" margin-bottom-5 mt10">
					...
					<span class="margin-left-10" style="font-size: 15px;">
						城市名称
						<input type="text" id="cityName" name="cityName" value="${cityName}"class="laydate-icon" style="width:150px;padding:5px;" />
					</span>
			    ...
			    <table class="table table-bordered table-striped table-hover">
		      	<tbody>
			        <tr align="center">
						...
						<td nowrap="nowrap" width="220px"><strong>城市拼音</strong></td>
	       			</tr>
		       		
		       		<c:forEach var="o" items="${CityList}" varStatus="s">					
					<tr align="center">
