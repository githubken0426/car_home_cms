...
<html xmlns="http://www.w3.org/1999/xhtml">
  ...
  <script type="text/javascript">
  	...
  	$(function(){
		//分页开始
  		...
  		kkpager.init({
  			...
  			hrefFormer : '${pageContext.request.contextPath}/feedback!listData',//链接前部
  			hrefLatter : '.action',		 //链接尾部
  			getLink : function(n){
  				return this.hrefFormer + this.hrefLatter + "?pno="+n+"&realName=${realName}&content=${content}"; //+"&groupId="+groupId;
  			},
  			lang : {
  				prePageText : '上一页',
  				nextPageText : '下一页',
	...
	//按条件查询
  	function query(){
  		$("#totalForm").attr("action","${pageContext.request.contextPath}/feedback!listData.action");
	  	$("#totalForm").submit();
  	}
  	...
  	function addDataPage(){  		
  		$("#totalForm").attr("action","${pageContext.request.contextPath}/feedback!addDataPage.action");
	  	$("#totalForm").submit();
  	}
	...
	//删除	
  	function deleteData(){  		
		...
		if(checkboxs.is(":checked")){
			if(confirm("确定要删除所选中项吗?")){
				$("#totalForm").attr("action","${pageContext.request.contextPath}/feedback!deleteData.action");
				$("#totalForm").submit();
			}
		}else{
			alert("请选择删除项！");
  	...
  	function updateDataPage(){
  		...
  		if (ret == 0) {
			...
			$("#totalForm").attr("action","${pageContext.request.contextPath}/feedback!updateDataPage.action");
			$("#totalForm").submit();	
  		}
  	}
  	...
  	function changeStatus(status){
  		...
  		if(checkboxs.is(":checked")){
	  		...
	  		$("#totalForm").attr("action","${pageContext.request.contextPath}/feedback!updateDataBatch.action");
	  		$("#totalForm").submit();
  		}else{
  			alert("请选择修改项！");
  		}
		...
		<form action="" method="post" id="totalForm" >
			...
			<div class="content-box">
			    <div class="content-box-header">
			    	<span class="now_location">当前位置:</span>[意见反馈]
			        <div class="clear"></div>
			    </div>
			    ...
			    <table class="table table-bordered table-striped table-hover">
		      	<tbody>
			        <tr align="center">
						...
						<td nowrap="nowrap" width="220px"><strong>用户</strong></td>
						<td nowrap="nowrap" width="220px"><strong>内容</strong></td>
						<td nowrap="nowrap" width="220px"><strong>提问时间</strong></td>
						<td nowrap="nowrap" width="220px"><strong>邮件</strong></td>
						<!-- <td nowrap="nowrap" width="220px"><strong>回复</strong></td> -->

	       			</tr>
		       		
		       		<c:forEach var="o" items="${FeedbackList}" varStatus="s">					
					<tr align="center">
